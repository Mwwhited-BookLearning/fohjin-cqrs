...
- **CQRS + Event Sourcing** at the core: every write is a `Command` handled against an
  ...
  never the reverse.
- **Two UI clients** talking to the same API: a retargeted WinForms desktop app and a
  Vue 3 SPA, both authenticating against a real (if minimal) OIDC identity provider.
...
- WinForms doesn't refresh its OIDC access token; it just expires after an hour with no
  silent renewal (`docs/09-winforms-ui.md`) — an accepted limitation for a dev sample.
